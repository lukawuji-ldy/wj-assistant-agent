...
主后端服务（默认端口 **8080**）：对外 Chat / SSE、Admin API，并装配 ReactAgent、MCP Client、Memory、RAG。本仓库两个可启动 Boot 应用之一。
...
- **管理 API**：`/api/admin/**`，Admin JWT（`token_type=ADMIN`）与聊天 JWT 双轨隔离
...
| 安全 | Spring Security + JJWT（User / Admin 双轨） |
| 数据 | PostgreSQL、Flyway、JDBC |
| 可观测 | Actuator、Micrometer Tracing、OpenTelemetry OTLP |
| 依赖模块 | `ai-agent-core`（进而 Memory / RAG / Common） |

设计见 [docs/architecture.md](../docs/architecture.md)、[docs/agent-flow.md](../docs/agent-flow.md)、[docs/admin-design.md](../docs/admin-design.md)。
